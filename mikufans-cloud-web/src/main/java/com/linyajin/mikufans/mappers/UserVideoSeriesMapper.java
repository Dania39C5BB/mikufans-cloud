...
public interface UserVideoSeriesMapper<T,P> extends BaseMapper<T,P> {
	...
	List<T> getUserVideoSeriesCover(@Param("userId") String userId);

	void changeVideoSeriesSort(@Param("userVideoSeriesList")List<T> userVideoSeriesList);

	List<T> findUserHomeVideoSeries(@Param("query")P p);
}
